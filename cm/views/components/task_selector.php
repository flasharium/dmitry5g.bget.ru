...
function process_mass_changing() {
    ...
    if (isset($_REQUEST['mass_changing'])) {
        ...
        if (count($task_ids)) {
            ...
            if ($new_user_id = array_get($_REQUEST, 'mass_changing.user_id')) {
                $changes['user_id'] = $new_user_id;
                $success = 'Задания ('.count($task_ids).') назначену КМу '. array_get(users(), $new_user_id) . '<br/>';
            }
...
function create_task_selector() {
    ...
    <div>
        <div class="form-group checkbox">
            <label>
                <input type="checkbox" onchange="var a='checked',c=$(this).prop(a);$('.task-checkbox').each(function(){$(this).prop(a,c)})"/>
                Выделить все
            </label>
        </div>
        ...
        <div class="form-group checkbox">
            ...
            <label>
                <select name="add_to_report[report_id][]">
                    <option value="">–</option>
                    <? foreach (reports(array('status' => REPORT_STATUS_NEW)) as $report) { ?>
